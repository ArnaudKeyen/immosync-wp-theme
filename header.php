...
<a class="sr-only focus:not-sr-only focus:absolute focus:left-4 focus:top-4 focus:z-[100] focus:bg-text-strong focus:px-4 focus:py-2 focus:text-surface" href="#wpis-content">
	<?php esc_html_e( 'Aller au contenu', 'hello-immosync' ); ?>
</a>

<header class="wpis-header sticky top-0 z-50 border-b border-border bg-surface/95 backdrop-blur" data-wpis-header>
		...
		<div class="flex items-center justify-between py-5">
			...
			<div class="wpis-brand">
				<?php if ( has_custom_logo() ) : ?>
					<?php the_custom_logo(); ?>
				<?php else : ?>
					<a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="font-display text-2xl tracking-tight text-text-strong">
						<?php bloginfo( 'name' ); ?>
					</a>
				<?php endif; ?>
			</div>
				...
				if ( has_nav_menu( 'primary' ) ) {
					wp_nav_menu(
						array(
							...
							'depth'          => 1,
							'link_before'    => '<span class="font-body text-sm tracking-wide text-text-strong transition-colors hover:text-brand">',
							'link_after'     => '</span>',
							'fallback_cb'    => false,
						)
					);
					...
					if ( $wpis_estates_link ) {
						printf(
							'<a class="font-body text-sm tracking-wide text-text-strong transition-colors hover:text-brand" href="%s">%s</a>',
			...
			<div class="flex items-center gap-4">
				...
				<button type="button" class="lg:hidden" data-wpis-menu-toggle aria-expanded="false" aria-controls="wpis-mobile-menu" aria-label="<?php esc_attr_e( 'Ouvrir le menu', 'hello-immosync' ); ?>">
					<span class="block h-px w-7 bg-text-strong"></span>
					<span class="mt-1.5 block h-px w-7 bg-text-strong"></span>
					<span class="mt-1.5 block h-px w-5 bg-text-strong"></span>
				</button>
			</div>
	...
	<!-- Menu mobile -->
	<div id="wpis-mobile-menu" class="hidden border-t border-border bg-surface lg:hidden" data-wpis-mobile-menu>
			...
			if ( has_nav_menu( 'primary' ) ) {
				wp_nav_menu(
					array(
						...
						'depth'          => 1,
						'link_before'    => '<span class="font-display text-2xl text-text-strong">',
						'link_after'     => '</span>',
						'fallback_cb'    => false,
					)
				);
