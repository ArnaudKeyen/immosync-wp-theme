...
<article class="wpis-card group">
	<a href="<?php echo esc_url( $wpis_link ); ?>" class="wpis-card-media block" aria-label="<?php echo esc_attr( wpis_get_title( $wpis_pid ) ); ?>">
		<?php if ( has_post_thumbnail( $wpis_pid ) ) : ?>
			...
			?>
		<?php else : ?>
			<span class="flex h-full w-full items-center justify-center text-text-muted">
				<?php echo wpis_icon( 'location', 'w-8 h-8' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			</span>
		<?php endif; ?>
	...
	<div class="flex flex-1 flex-col px-5 py-5">
		...
		<div class="mt-5 flex items-center justify-between">
			<p class="font-display text-xl text-text-strong"><?php echo esc_html( wpis_get_price( $wpis_pid ) ); ?></p>
