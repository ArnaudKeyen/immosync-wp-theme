...
<article class="wpis-card group">
	<a href="<?php echo esc_url( $wpis_link ); ?>" class="wpis-card-media block" aria-label="<?php echo esc_attr( wpis_get_title( $wpis_pid ) ); ?>">
		<?php if ( has_post_thumbnail( $wpis_pid ) ) : ?>
			...
			?>
		<?php else : ?>
			<span class="flex h-full w-full items-center justify-center text-mist"><?php echo wpis_icon( 'location', 'w-8 h-8' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
		<?php endif; ?>
	...
	<div class="flex flex-1 flex-col px-1 pt-5">
		<p class="wpis-eyebrow mb-2 flex items-center gap-2 text-stone">
			...
			echo esc_html( implode( '  ·  ', array_filter( array( $wpis_cat, $wpis_loc ) ) ) );
			?>
		</p>

		<h3 class="font-display text-2xl leading-snug text-ink">
			<a href="<?php echo esc_url( $wpis_link ); ?>" class="transition-colors hover:text-brand"><?php echo esc_html( wpis_get_title( $wpis_pid ) ); ?></a>
		</h3>

		<?php $wpis_excerpt = wpis_get_excerpt( $wpis_pid, 18 ); ?>
		<?php if ( '' !== $wpis_excerpt ) : ?>
			<p class="mt-2 text-sm leading-relaxed text-stone"><?php echo esc_html( $wpis_excerpt ); ?></p>
		<?php endif; ?>
			...
			<ul class="mt-5 flex flex-wrap items-center gap-x-5 gap-y-2 border-t border-line pt-4 text-sm text-charcoal">
				<?php foreach ( $wpis_features as $wpis_feature ) : ?>
					<li class="flex items-center gap-1.5">
						<span class="text-brand"><?php echo wpis_icon( $wpis_feature['icon'], 'w-4 h-4' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
		...
		<div class="mt-5 flex items-center justify-between">
			<p class="font-display text-xl text-ink"><?php echo esc_html( wpis_get_price( $wpis_pid ) ); ?></p>
